...
import { StackParamList } from './screenName';
import type { NavigationProps } from './RootNavigator';
...
export function getActiveRouteName(
  ...
  if (!route.state) return route.name as keyof StackParamList;

  // Recursive call to deal with nested routers
  return getActiveRouteName(route.state as NavigationState<StackParamList>);
}
...
export function useNavigationPersistence(
  ...
  const routeNameRef = useRef<keyof StackParamList | undefined>();
  ...
  const onNavigationStateChange = (state: NavigationState | undefined) => {
    ...
    if (state !== undefined) {
      ...
      routeNameRef.current = currentRouteName as keyof StackParamList;
